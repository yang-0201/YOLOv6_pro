...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        # self.m = nn.ModuleList(DecoupledHead_yolov6(x, nc, anchors) for x in ch)
        self.inplace = inplace  # use in-place ops (e.g. slice assignment)
        # self.dy1 = DyHead(256,256,6,False)
        # self.dy2 = DyHead(512, 512, 3, False)
        # self.dy3 = DyHead(1024, 1024, 3, False)


    def forward(self, x):
        z = []  # inference output
        # x[0] = self.dy1(tuple([x[0]]))
        # x[1] = self.dy2(tuple([x[1]]))
        # x[2] = self.dy3(tuple([x[2]]))
...
class Model(nn.Module):
    ...
    def _forward_augment(self, x):
        ...
        s = [1, 0.83, 1.25,1.5]  # scales
        f = [None, 3,3,None]  # flips (2-ud, 3-lr)
        y = []  # outputs
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # torchvision
            ...
            c2 = args[0]
        elif m in [SwinTR,SwinTRV2]:
            c2 = args[2]
        elif m in [swin_permute,ERB,BepC3,CSPNeXtLayer]:
            c2 = args[1]
        elif m in [ERB_down]:
            c2 = args[2]
        elif m in [E_ELAN]:
        ...
        #             args[1] = i
        #             args[2] = j
        #             cbs_save.append(m(*args))
        #             model = nn.Sequential(m(*args))
        #             m_.add_module(str(i)+str(j),m(*args))
